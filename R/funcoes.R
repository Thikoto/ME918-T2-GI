...
preditos = function(x, betas) {
  ...
  if (!is.matrix(betas) && !is.data.frame(betas)) {
    stop("betas deve ser uma matriz ou data frame numérico.")
  }
...
residuos = function(y, preditos) {
  ...
  if (!is.numeric(preditos)) {
    stop("preditos deve ser um vetor numérico.")
  }
  # Calcula os resíduos
  residuo = y - preditos
...
novos_preditos = function(novox, betas) {
  ...
  if (!is.matrix(novox) && !is.data.frame(novox)) {
    stop("novox deve ser uma matriz ou data frame numérico.")
  }
  if (!is.matrix(betas) && !is.data.frame(betas)) {
    stop("betas deve ser uma matriz ou data frame numérico.")
  }
